...
Project Files
.
├── sync_drive_LED.py      # Main integration script: tracking + distance + motor control
├── motor_control.py       # Velocity/stop control logic based on distance threshold
├── led_tracking.py        # Image differencing + normalized correlation tracking
├── distance_calc.py       # Pixel-to-inch conversion using regression coefficients
├── README.md              # This file
Results
...
Wheel speeds constrained to 0.1 m/s to stay within hardware limits (0.22 m/s) and prevent stalling during turns
Dependencies
ROS Noetic
Python 3
...
Activate the LED Raspberry Pi node for initial blinking:
rosrun your_package_name led_activate.py
Run the integrated tracking, distance estimation, and control script:
